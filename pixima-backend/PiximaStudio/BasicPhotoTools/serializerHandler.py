...
class ImageSerializerHandler(SerializerHandler):
    def __init__(self, serializer: serializer.ImageHandlerSerializer,preview_optinos:list=None) -> None:
    ...
    def handle(self) -> bool:
        ...
        if res:
            if (
                ...
                self.errors = {
                    "Message" : "Both id and Image Can't Be Null"
                }
                return False
            if self.serializer.data["ImageIndex"] < 0:
                self.errors = {
                    "Message" : "ImageIndex Less Than 0"
                }
                return False
